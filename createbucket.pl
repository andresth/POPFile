...
# ----------------------------------------------------------------------------
#
# createbucket.pl --- Creates a bucket
#
...
if ( $#ARGV == 0 ) {
    ...
    if ( $POPFile->CORE_config() ) {
        ...
        if ( !$b->create_bucket( $session, $bucket ) ) {
          print STDERR "Error: Bucket `$bucket' was not created.\n";
          $code = 1;
        } else {
          print "Bucket `$bucket' was created.\n";
        }
    ...
    print "createbucket.pl - creates a bucket\n\n";
    print "Usage: createbucket.pl <bucket> <messages>\n";
